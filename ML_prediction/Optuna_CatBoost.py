...
def objective(trial):
    ...
    cat_features = []
    catboost.fit(X_train_scaled, y_train, cat_features=cat_features)
...
best_catboost.fit(X_train_scaled, y_train, cat_features=[])
...
train_data.to_csv('data/CamFreqSyn/train_data_CatBoost_R2.csv', index=False)

test_data = pd.DataFrame({
    'Actual': y_test,
    'Predicted': y_test_pred
})
test_data.to_csv('data/CamFreqSyn/test_data_CatBoost_R2.csv', index=False)
